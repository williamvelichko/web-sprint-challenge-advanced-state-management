...
1. What problem does the context API help solve?
   context API solves a easier way to share data throughout the application instead of prop drilling. in this case we dont even need to use props at all.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions are packets of informations that contain action type and data. Actions are dispatch type that are passed in the reducer as an argument.
Reducers are pure functions that takes an action and the previous state and returns a new state.
Store is a data holding package. Store is where the all the data is held. The store is like a shopping basket where all your items are in.
The store is known as a single source of truth in the redux application becasause on every component you can grab the state stait from the store and not use prop drilling threw multiple components.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
   redux-thunk helps us use actions asynchronously so instead of using one action at a time this will use multiple actions in on time.

4. What is your favorite state management system you've learned and this sprint? Please explain why!
   My favorite would definetly be using redux becasause its really accurate and makes coding alot easier when you have all the state stored in the store.
